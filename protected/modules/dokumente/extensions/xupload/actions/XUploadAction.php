...
class XUploadAction extends CAction {
    ...
    /**
     * Path of the folder to save the file.
     * @see XUploadAction::init()
     * @var string
     * @since 0.1
     */
    ...
    public function init( ) {
         ...
         if( Yii::app()->user->hasState('path') ) {
            $this->path = Yii::app()->user->getState('path');
    ...
    protected function handleUploading()
    {
        ...
        if ($model->{$this->fileAttribute} !== null) {
            ...
            if ($model->validate()) {
                ...
                $model->{$this->fileAttribute}->saveAs($path .DIRECTORY_SEPARATOR. $model->{$this->fileNameAttribute});
                chmod($path .DIRECTORY_SEPARATOR. $model->{$this->fileNameAttribute}, 0777);
